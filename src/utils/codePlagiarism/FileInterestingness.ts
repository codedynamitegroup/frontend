...
export type LongestFragmentScore = {
...
export class FileInterestingnessCalculator {
  ...
  public calculateSimilarityScore(file: File): SimilarityScore | null {
    ...
    const pair = pairArray.reduce((a, b) => (a.similarity > b.similarity ? a : b));
  ...
  public totalOverlapScore(file: File): TotalOverlapScore | null {
    ...
    const pair = pairArray.reduce((a, b) => (a.totalOverlap > b.totalOverlap ? a : b));
  ...
  public longestFragmentScore(file: File): LongestFragmentScore | null {
    ...
    const pair = pairArray.reduce((a, b) => (a.longestFragment > b.longestFragment ? a : b));
...
export function getLargestPairOfScore(scoredFile: FileScoring): Pair | null {
  const scores = [
    scoredFile.similarityScore,
    scoredFile.longestFragmentScore,
    scoredFile.totalOverlapScore
  ];

  return (
    scores.reduce((s, ns) => ((s?.weightedScore || 0) > (ns?.weightedScore || 0) ? s : ns))?.pair ||
    null
  );
}
