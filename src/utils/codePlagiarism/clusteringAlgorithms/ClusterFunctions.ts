...
// Refactor: Use Map to bypass Set is not working with objects because it uses reference equality
export function getClusterElements(cluster: Cluster): Set<File> {
  const map = new Map<number, File>();
  cluster.forEach((v) => map.set(v.leftFile.id, v.leftFile));
  cluster.forEach((v) => map.set(v.rightFile.id, v.rightFile));

  const set = new Set<File>();

  // cluster.forEach((v) => set.add(v.leftFile));
  // cluster.forEach((v) => set.add(v.rightFile));
  map.forEach((v) => set.add(v));
 ...
 */
export function getClusteringGraph(pairs: Set<Pair>, similarity = 0): ClusteringGraph {
