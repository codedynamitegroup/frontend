...
const UpdateCertificateCourse = () => {
  ...
  const [certificateCourseData, setCertificateCourseData] = useState<CertificateCourseEntity>();
  const [certificateCourseChapter, setCertificateCourseChapter] = useState<ChapterEntity[]>([]);
  ...
  const getCertificateCourseData = async () => {
    try {
      if (id) {
        const response = await CertificateCourseService.getCertificateCourseById(id);
        setCertificateCourseData(response);
      }
    } catch (error) {
      console.error("error", error);
    }
  };
  const getCertificateCourseChapter = async () => {
    try {
      if (id) {
        const response = await ChapterService.getChaptersByCertificateCourseIdResponse(id);
        setCertificateCourseChapter(response.chapters);
      }
    } catch (error) {
      console.error("error", error);
    }
  ...
  useEffect(() => {
    reset({
      name: certificateCourseData?.name || "",
      description: certificateCourseData?.description || "",
      skillLevel: certificateCourseData?.skillLevel || skillLevelOptions[0].value,
      topicId: certificateCourseData?.topic.topicId || topicList[0]?.topicId,
      chapters: certificateCourseChapter?.map((chapter, index) => {
        return {
          no: index,
          title: chapter.title,
          description: chapter.description,
          resources: chapter.resources.map((resource, idx) => {
            return {
              no: idx,
              resourceType: resource.resourceType,
              title: resource.title,
              ...
              lessonHtml: resource.lessonHtml,
              youtubeVideoUrl: resource.youtubeVideoUrl
            };
          })
        };
      })
    });
  }, [certificateCourseData, certificateCourseChapter]);
        ...
        <Container>
          ...
          >
            <CustomBreadCrumb
              ...
              lastBreadCrumbLabel='Edit Certificate Course'
            />
          </Box>
          <Grid
            container
          ...
          >
            <Grid item xs={12}>
              <Heading3 translate-key='create_certificate_course_title'>
                {t("edit_certificate_course_title").toUpperCase()}
              </Heading3>
            </Grid>
...
export default UpdateCertificateCourse;
