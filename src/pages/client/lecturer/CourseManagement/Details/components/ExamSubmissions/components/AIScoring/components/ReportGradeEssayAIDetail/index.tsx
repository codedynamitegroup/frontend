...
export default function ReportGradeEssayAIDetail() {
  ...
  const { courseId, assignmentId, submissionId, reportId } = useParams<{
    courseId: string;
    assignmentId: string;
    submissionId: string;
    reportId: string;
  ...
  const [listStudentSubmissionsFeedback, setListStudentSubmissionsFeedback] = React.useState<any[]>(
    []
  );
  const [listStudentSubmissions, setListStudentSubmissions] = React.useState<any[]>([]);
  ...
  const getCourseById = useCallback(
    ...
    [courseState.courseDetail, dispatch]
  );
  ...
  const handleGetReportDetail = useCallback(
    async (reportId: string) => {
      dispatch(setLoading(true));
      try {
        const response = await AssignmentService.getAIReportDetail(reportId);
        setListStudentSubmissionsFeedback(JSON.parse(response.feedbackSubmissions));
        setListStudentSubmissions(JSON.parse(response.studentSubmissions).submissionAssignments);
      } catch (error) {
...
`;
  ...
  const assignmentFeedbackAI = listStudentSubmissionsFeedback.find((submission) => {
    return submission.studentSubmissionId === submissionId;
  });
        ...
        <>
          ...
          >
            ...
            >
              ...
              {assignmentState.assignmentDetails?.type === "BOTH" ? (
                <>
                  <Box className={classes.drawerFieldContainer} translation-key='no_data'>
                    ...
                    <MDEditor.Markdown
                      source={assignmentSubmitStudent?.studentAnswer}
              ...
              <Box className={classes.drawerFieldContainer}>
                <TextTitle translation-key='course_lecturer_score_on_range_by_ai'>
                  {t("course_lecturer_score_on_range_by_ai", { range: 100 })}
                </TextTitle>
                <InputTextField
                  type='number'
                  value={assignmentFeedbackAI?.score_overall}
              ...
              <Box className={classes.feedbackContainer}>
                <Box className={classes.textEditor}>
                  <TextTitle translation-key='course_lecturer_grade_comment_by_ai'>
                    {t("course_lecturer_grade_comment_by_ai")}
                  </TextTitle>
                  <Box data-color-mode='light'>
                    <MDEditor.Markdown
                      source={assignmentFeedbackAI?.feedback}
                      className={classes.markdown}
                    />
                  </Box>
