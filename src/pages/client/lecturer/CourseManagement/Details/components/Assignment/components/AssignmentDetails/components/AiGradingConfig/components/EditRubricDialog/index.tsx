...
import { close, open as openSelectRubricDialog } from "reduxes/SelectRubricDialog";
...
const EditRubricDialog = ({ isOpen, onClose, previewRubric }: PropsData) => {
  ...
  useEffect(() => {
    if (previewRubric) {
      reset({
        ...
        description: previewRubric.description,
        criteria: criteria
      });
    }
  }, [previewRubric, reset]);
  ...
  const onSave = async (data: any) => {
    ...
    const updateRubricUserCommandData: UpdateRubricUserCommand = {
      rubricUserId: previewRubric?.id,
      rubricName: data.name,
      rubricDescription: data.description,
      rubricContent: JSON.stringify(data.criteria)
    };

    await RubricUserService.updateRubricUser(updateRubricUserCommandData)
      .then((res) => {
        dispatch(setSuccessMess("Rubric updated successfully"));
        dispatch(close());
        onClose();
      })
      .catch((err) => {
        dispatch(setErrorMess("Failed to update rubric"));
        ...
        <AppBar sx={{ position: "fixed", backgroundColor: "white" }}>
          <Box>
            <Toolbar ref={stickyHeaderRef}>
              <Heading2 sx={{ flex: 1 }} translation-key='grading_config_add_edit_rubric'>
                {t("grading_config_add_edit_rubric")}
              </Heading2>
...
export default EditRubricDialog;
