...
const EditEssayQuestion = (props: Props) => {
  ...
  const [essayQuestionData, setEssayQuestionData] = useState<EssayQuestion>();
  ...
  const handleGetEssayQuestionDetailForm = async (questionId: string) => {
    try {
  ...
  const submitHandler = async (data: any) => {
    ...
    const newEssayQuestion: PutEssayQuestion = {
      qtEssayQuestionId: essayQuestionData?.id || "",
      responseFormat: formSubmittedData.responseFormat,
      responseRequired: Number(formSubmittedData.responseRequired) || 0,
      responseFieldLines: Number(formSubmittedData.responseFieldLines) || 10,
      minWordLimit: Number(formSubmittedData.minWord) || -1,
      maxWordLimit: Number(formSubmittedData.maxWord) || -1,
      attachments: Number(formSubmittedData.attachments) || 0,
      attachmentsRequired: Number(formSubmittedData.attachmentsRequired) || 0,
      graderInfo: formSubmittedData.graderInfo || "",
      graderInfoFormat: "1",
      responseTemplate: formSubmittedData.responseTemplate || "",
      maxBytes: Number(formSubmittedData.maxBytes) || 10,
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (questionId) {
        ...
        setEssayQuestionData(res.questionResponses[0].qtypeEssayQuestion);
      }
    };
  ...
  useEffect(() => {
    if (essayQuestionData) {
      setValue("questionName", essayQuestionData.question.name);
      setValue("questionDescription", essayQuestionData.question.questionText);
      setValue("defaultScore", essayQuestionData.question.defaultMark.toString());
      setValue("generalDescription", essayQuestionData.question.generalFeedback);
      setValue("responseFormat", essayQuestionData.responseFormat);
      setValue("responseRequired", essayQuestionData?.responseRequired?.toString() || "0");
      setValue("responseFieldLines", essayQuestionData.responseFieldLines.toString());
      setValue("attachments", essayQuestionData.attachments.toString());
      setValue("attachmentsRequired", essayQuestionData?.attachmentsRequired?.toString() || "0");
      setValue("graderInfo", essayQuestionData.graderInfo);
      setValue("responseTemplate", essayQuestionData.responseTemplate);
      setValue("minWord", essayQuestionData.minWordLimit);
      setValue("maxWord", essayQuestionData.maxWordLimit);
      setValue("maxBytes", essayQuestionData?.maxBytes?.toString() || "10");
        ...
        <>
          <Grid className={classes.root}>
            ...
            <Container style={{ marginTop: `${headerHeight}px` }} className={classes.container}>
              ...
              <form
                onSubmit={handleSubmit(submitHandler, () => setSubmitCount((count) => count + 1))}
              >
                                  ...
                                  >
                                    {!selectedFileTypes.includes("all") ? (
                                      ...
                                      <Option value={fileTypes[0].type}>
                                        <Checkbox
                                          checked={
                                            selectedFileTypes.indexOf(fileTypes[0].type) > -1
                                          }
                                        />
                                        <ListItemText
